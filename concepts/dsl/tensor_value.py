...
try:
    import jactorch  # type: ignore
except ModuleNotFoundError:  # pragma: no cover
    from types import SimpleNamespace

    class _JactorchBatchSentinel:
        pass

    _batch_sentinel = _JactorchBatchSentinel()

    def _bvindex(tensor: torch.Tensor):
        class _Indexer:
            def __init__(self, t: torch.Tensor):
                self._t = t

            def __getitem__(self, indices):
                if indices is _batch_sentinel:
                    indices = slice(None)
                elif isinstance(indices, tuple):
                    indices = tuple(slice(None) if i is _batch_sentinel else i for i in indices)
                return self._t[indices]

        return _Indexer(tensor)

    jactorch = SimpleNamespace(batch=_batch_sentinel, bvindex=_bvindex)
